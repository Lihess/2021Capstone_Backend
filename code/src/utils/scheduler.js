...
const createIIRS = () => {
    ...
    const query = `
        SELECT (CASE @GROUPING WHEN c.ingrOrnu+c.refNum THEN @RANK := @RANK + 1 ELSE @RANK := 1 END) AS recipeOrnu, (@GROUPING:=ingrOrnu+refNum),
        ...
        FROM (select x.recipe_num as recipeNum, y.ingr_ornu as ingrOrnu,  y.ref_num as refNum
	        ...
	        order by y.ref_num, y.ingr_ornu) c, (SELECT @GROUPING := '', @RANK := 0) R;
    `
